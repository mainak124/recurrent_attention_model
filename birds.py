...
#image_size  = 227
image_size  = 256 if context_net_type == "alex" else 227
num_step = 3
lambda_reward = 0.7
n_sample = 10
n_classes = 555
...
def glimpse_net(x, locs, keep_prob, is_train, sample_idx, glimpse_size=16):
    ...
    #x2 = tf.image.resize(tf.image.extract_glimpse(x, size=[2*glimpse_size, 2*glimpse_size], offsets=loc), [[1,glimpse_size, glimpse_size, 3]])
    # FIXME: use multiple glimpses and make sure conv2d works for that !
    ...
    # print "PATCH SHAPE: ", patch.get_shape()
    #patch = x1

    with tf.variable_scope("conv1"):
        params_conv1, conv1 = _conv(patch, nIn=6, nOut=16, ker=3, st=1, padType='VALID', bias_val = 0.1)
...
def build_net(x, y_, keep_prob, optimizer, learning_rate, batch_cnt, sess):
 ...
 #               locs_grads.append(locs_g)
                ...
                for grad in locs_g:
                    grads_1b_prod.append(lambda_reward * tot_reward * grad)
...
def train_net(dataset_path = '/raid/mainak/datasets/nabirds/', image_path = 'images',n_classes=555): 
    ...
    learning_rate = tf.train.exponential_decay(0.01, global_step=batch_cnt, decay_steps=5000, decay_rate=0.7, staircase=True)
